...
window.JCCatalogElement = function (arParams)
{
	...
	this.startTypeUnit = $('.calc-measures a.active[data-unit]').data('unit');
		...
		if (!isNaN(curValue))
		{
			...
			if (boolSet)
			{
				...
				if ($unitRecalc) {
					...
					recalcUnits(true, this.startTypeUnit);
				}
				/*---end 2017-04-14---*/
			}
		}
	...
	if ('OK' === arResult.STATUS)
	{
		...
		buttons = [
			...
			new BX.PopupWindowButton({
				...
				className: 'popup-btn-continue-shopping',
				events: {
...
jQuery(document).ready(function() {
	var startTypeUnit = $('.calc-measures a.active[data-unit]').data('unit');
	...
	jQuery('#unit-quantity').keyup(function() {
		var activeUnit = $(this).closest('.item_buttons').find('a.active[data-unit]').data('unit');
		var sqr = $(this).closest('.item_buttons').find('.calc-measures').data('sqr');
		var curVal = parseFloat($(this).val());

		if (activeUnit === 'i' && !isNaN(curVal))
		{
			curVal = Math.ceil(curVal);
			$(this).val(curVal);
		}
		/*
		if (activeUnit === startTypeUnit)
			newprice1 = Math.round(newprice * curVal * 100) / 100;
		else if (activeUnit === 'i')
			newprice1 = Math.round(newprice * sqr * curVal * 100) / 100;
		else
			newprice1 = Math.round(newprice / sqr * curVal * 100) / 100;
 
		jQuery('.new-price').html(newprice1 + ' <span class="new-ci">руб.</span>');
		*/
		recalcUnits(true, startTypeUnit);
	});
});
/*---end 2017-04-14---*/
...
jQuery(document).ready(function() {
	...
	var newprice=jQuery('#cena').html();
	var sqr = jQuery('.calc-measures').attr('data-sqr');
	//sqr = Math.round(sqr * 10000) / 10000;
	var pac = jQuery('.calc-measures').attr('data-pac');
	var startTypeUnit = $('.calc-measures a.active[data-unit]').data('unit');
	...
	jQuery('.bx_bt_button_type_2').click(function() {
		...
		/*if(jQuery('a[data-unit="i"]').hasClass('active')){
			...
			if (startTypeUnit === 'i')
				newprice2 = Math.round(newprice * kolvo * 100) / 100;
			else
		...
		/*if(jQuery('a[data-unit="m"]').hasClass('active')){
			...
			if (startTypeUnit === 'i')
				newprice3 = Math.round(newprice / kef * kolvo * 100) / 100;	
			else
				newprice3 = Math.round(newprice * kolvo * 100) / 100;
	...
	/*переключение на упак*/
	jQuery('.calc-measures a[data-unit="p"]').click(function() {
		recalcUnits(true, startTypeUnit);
		
		if (startTypeUnit === 'm')
			newprice1 = Math.round(newprice * sqr * pac);	
		else
			newprice1 = Math.round(newprice * 100) / 100;

		jQuery('.new-price').html(newprice1+' <span class="new-ci">руб./упак.</span>');
		jQuery('.item_price>.new-ci').html('');
		jQuery('#testizm').html('уп.');
	});	
	/*переключение на упак*/
});	