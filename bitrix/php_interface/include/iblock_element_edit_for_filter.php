...
if(!empty($PROP)):
	...
	foreach($PROP as $prop_code=>$prop_fields):
		/*---bgn 2015-07-09---*/
		/*if (in_array($prop_code, array('FLTR_46','FLTR_44'))) {
			switch($prop_code) {
				case 'FLTR_46':
					$fltr_prop_id = 46; //поверхность
					break;
				case 'FLTR_44':
					$fltr_prop_id = 44; //цвет
					break;
			}*/
		if (substr_count($prop_code, 'FLTR_') > 0) {
			$fltr_prop_id = str_replace('FLTR_', '', $prop_code);
			//получаем значения св-ва для описаний
			...
			$fltr_prop_enums = CIBlockProperty::GetPropertyEnum($fltr_prop_id);
			while($arFPE = $fltr_prop_enums->Fetch()) {
				$val = '['.$arFPE['ID'].'] '.$arFPE['VALUE'];
				if (count($arDescEnums) > 0) {
					$has = false;
					foreach($arDescEnums as $arSE) {
						if ($arSE['VALUE'] == $val) {
							$has = true;
							break;
						}
					...
					if (!$has) {
						CIBlockPropertyEnum::Add(array('PROPERTY_ID'=>$prop_fields['ID'], 'VALUE'=>$val));
					}
				} else {
					CIBlockPropertyEnum::Add(array('PROPERTY_ID'=>$prop_fields['ID'], 'VALUE'=>$val));
				}
